...
export class VoisResourcesComponent {
  ...
  /* ── Custom resources (persisted in localStorage) ── */
  ...
  get canSaveResource(): boolean {
    return (
      this.newLabel.trim().length > 0 &&
      this.resolvedCategory.length > 0
    );
  }

  get allCategoryNames(): string[] {
    const cats = new Set<string>(this.defaultResources.map(r => r.category));
  ...
  private defaultResources: VoisResource[] = [
    // ── Cloud & AWS ──
    {
      label: 'AWS Accounts',
  ...
  get resources(): VoisResource[] {
    return [...this.defaultResources, ...this.customResources];
  }
